...
// Minimal helpers
const mm = (v:number)=>v; // (units already normalized to mm by /api/spec)
...
function planJoineryFromSpec(spec: Spec){
  ...
  // Side panel dims (host for dados/grooves): height x depth
  ...
  if (spec.assembly.joinery_policy.back === 'rabbet') {
    const rabWidth = Math.min(12, Math.round(D * 0.05));     // ~12mm or small fraction of depth
    const rabDepth = Math.min(Math.round(sideThk * 0.6), backThk);
    plates.push({
      kind:'rabbet',
      spec:{
        units: 'mm',
        host: { name: 'Side Panel', thickness: sideThk },
        insert: { name: 'Back Panel', thickness: backThk },
    ...
    const grooveW = Math.min(backThk, 6.35); // if bit is 1/4", grooves <= bit become SLOT
    const grooveD = Math.min(Math.round(sideThk * 0.5), backThk);
    const offsetY = Math.round(H * 0.5); // centered
    plates.push({
      kind:'groove',
      spec:{
        units:'mm',
        host: { name:'Side Panel', thickness: sideThk, length: H, width: D },
  ...
  if (spec.assembly.joinery_policy.shelves === 'dado') {
    const shelfOffset = Math.round(H * 0.5); // one shelf in the middle (simple demo)
    const dadoW = shelfThk;
    const dadoD = Math.min(Math.round(sideThk * 0.35), 8); // ~1/3 thickness or 8mm
    plates.push({
      kind:'dado',
      spec:{
        units:'mm',
  ...
  return { plates, jobs, toolDefaults:{
    units:'mm',
    tooling:{ endmillDiameter: 6.35, stepdown: 2, stepover: 0.5, feedXY: 900, feedZ: 300, safeZ: 5 }
  }};
}
...
export default function OneClick() {
  ...
  const onGenerate = async () => {
    if (once.current) { /* no dup runs in Strict Mode */ } else { once.current = true; }
    setLoading(true);
    setSpec(null); setPlan(null); setDebug(null);
    try {
      ...
      const p = planJoineryFromSpec(data.spec);
      setPlan(p);
    } catch (e:any) {
      alert(e.message || String(e));
    } finally {
      setLoading(false);
      setTimeout(()=>{ once.current=false; }, 100); // allow another run
    }
  };
